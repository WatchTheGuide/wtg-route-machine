...
vi.mock('@ionic/react', () => ({
  IonActionSheet: ({ isOpen, buttons, onDidDismiss }: { 
    isOpen: boolean; 
    ...
    isOpen ? (
      <div data-testid="action-sheet">
        {buttons.map((btn, i) => (
          <button key={i} onClick={() => { btn.handler?.(); onDidDismiss(); }}>
            {btn.text}
          </button>
        ))}
      </div>
  ...
  IonButton: ({ children, onClick, ...props }: React.PropsWithChildren<{ onClick?: () => void }>) => (
